...
Refactored from class to functions for better Streamlit caching integration.
"""
...
def get_raw_products() -> pd.DataFrame:
    ...
    try:
        conn = get_conn()
        df_raw = conn.query(SQL_QUERY_PRODUCTS)
...
def transform_products(df_raw: pd.DataFrame) -> pd.DataFrame:
...
def get_all_products() -> pd.DataFrame:
    """Load & transform data produk (convenience function).

    Calls get_raw_products() + transform_products()
    ...
    """
    df_raw = get_raw_products()

    if df_raw.empty:
        return df_raw

    df = transform_products(df_raw)
